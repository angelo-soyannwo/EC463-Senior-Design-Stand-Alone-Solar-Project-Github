...
async def calibrateRTC():
    #Update Pico RTC datetime on startup
    #Change WIFI ssid and password in RTC_wifi module
    #await uasyncio.sleep(2)
    await RTC_wifi.connectWIFI()
    rtc = RTC()  
    await RTC_wifi.setTimeRTC(rtc)      # set time, from RTFC_wifi module
    led.high()              #show successful calibration
    
async def connectScreen(sec):
    print("1")
    asyncMC.lcd_settings()
    lcd.move_to(0, 0)
    lcd.putstr("Calibrating")
    lcd.move_to(0,1)
    lcd.putstr("Real Time")
    for x in range(sec):
        lcd.putstr(".")
...
async def rotateMotor(steps, direction, delay, currSteps):
...
def TFoperations(timeFactor, top_pauseTime, total_steps, dayStart, dayEnd):
    ...
    #0.9 to make the delay slightly shorter than exact timeframe, for leeway during time reads
    delay = 0.9*(timeframe - TFtop_pauseTime)/(total_steps*4)  #Example 2sec/(200steps*2) = 0.005 time between each step  
...
#                 print("targetSteps: ", targetSteps)
        ...
        if targetSteps >= 0 and targetSteps <= total_steps:
            if currSteps < targetSteps:
                currSteps = await rotateMotor(targetSteps - currSteps, CW, delay, currSteps)
            elif currSteps > targetSteps:
                currSteps = await rotateMotor(currSteps - targetSteps, CCW, delay, currSteps)
...
async def main():
    ...
    try:
        connectScreen_task = uasyncio.create_task(connectScreen(5))
        await calibrateRTC()
        #await uasyncio.create_task(calibrateRTC())
        
    except:
        connectScreen_task.cancel()
